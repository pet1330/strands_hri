...
class OnlineQTCCreator(object):
    ...
    def ppl_callback(self, msg):
        ...
        for idx, (uuid, pose) in enumerate(zip(msg.uuids, msg.poses)):
            person = PoseStamped()
            ...
            for (uuid2, pose2) in zip(msg.uuids[idx+1:], msg.poses[idx+1:]):
                person2 = PoseStamped()
                person2.header = msg.header
                person2.pose = pose2
                try:
                    self.listener.waitForTransform(msg.header.frame_id, self.target_frame, msg.header.stamp, rospy.Duration(1.0))
                    transformed2 = self.listener.transformPose(self.target_frame, person2)
                except (tf.Exception, tf.LookupException, tf.ConnectivityException) as ex:
                    rospy.logwarn(ex)
                    return
                
                if not uuid+uuid2 in self._buffer.keys():
                    self._buffer[uuid+uuid2] = {"data": np.array(
                        [
                            transformed.pose.position.x,
                            transformed.pose.position.y,
                            transformed2.pose.position.x,
                            transformed2.pose.position.y
                        ]
                    ).reshape(-1,4), "last_seen": msg.header.stamp.to_sec()}
                else:
                    self._buffer[uuid+uuid2]["data"] = np.append(
                        self._buffer[uuid+uuid2]["data"],
                        [
                            transformed.pose.position.x,
                            transformed.pose.position.y,
                            transformed2.pose.position.x,
                    ...
                    if self._buffer[uuid+uuid2]["data"].shape[0] > 3:
                        self._buffer[uuid+uuid2]["data"] = self._buffer[uuid+uuid2]["data"][:-1] if np.allclose(self._buffer[uuid+uuid2]["data"][-1], self._buffer[uuid+uuid2]["data"][-3]) else self._buffer[uuid+uuid2]["data"]
                self._buffer[uuid+uuid2]["last_seen"] = msg.header.stamp.to_sec()

                if self._buffer[uuid+uuid2]["data"].shape[0] > 2:
                    qtc = self.input.convert(
                        data=self.input.generate_data_from_input(
                            agent1="Robot",
                            agent2="Human",
                            x1=self._buffer[uuid+uuid2]["data"][:,0],
                            y1=self._buffer[uuid+uuid2]["data"][:,1],
                            x2=self._buffer[uuid+uuid2]["data"][:,2],
                            y2=self._buffer[uuid+uuid2]["data"][:,3]
                        ),
